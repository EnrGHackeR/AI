  ...
  return (
    <section id="features" className="py-32 px-6 relative overflow-hidden">
      <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-center pointer-events-none z-0">
        <span className="font-bold text-center text-[20vw] sm:text-[18vw] md:text-[16vw] lg:text-[14vw] leading-none tracking-tighter text-white/[0.03] whitespace-nowrap">
          MANAGE
        </span>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-1 lg:order-2 space-y-8">
            ...
            >
              <TypewriterHeading
                text="Manage your rentals with ease"
                tag="h2"
                className="text-4xl md:text-5xl font-normal mb-6 text-balance font-serif text-[#36B1C7]"
                typeSpeed={35}
              />
              <p className="text-[#E8D8C3]/70 leading-relaxed text-lg">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {features.map((feature, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="flex items-center p-3 rounded-xl hover:bg-white/[0.03] transition-colors duration-300 gap-2 py-1"
                >
                  <div className="w-6 h-6 bg-gradient-to-br from-[#C1121F] to-[#FF2D55] rounded-full flex items-center justify-center flex-shrink-0 shadow-md">
                    <Check className="w-3.5 h-3.5 text-white" strokeWidth={2.5} />
                  </div>
                  <span className="text-sm text-[#FBF6EE]">{feature}</span>
                </motion.div>
              ))}
            </div>
          </div>
